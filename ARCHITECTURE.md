...
8. Model selection: `createCompletion` walks an ordered fallback list (`DEFAULT_OPENROUTER_MODELS`, overridable via `OPENROUTER_MODEL`); HTTP 429 and timeouts advance to the next model, other failures stop immediately. When image references are present, the list is filtered to vision-capable models.
